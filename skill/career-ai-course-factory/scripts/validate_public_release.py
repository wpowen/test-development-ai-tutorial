...
def validate_release(root: Path) -> list[str]:
    ...
    fixture_preview = manifest.get("release_profile") == "fixture-preview.v1"
    fixture_preview_blocker_valid = False
    if fixture_preview:
        blocker_ref = manifest.get("publication_blockers_ref")
        blocker_path = (root / str(blocker_ref)).resolve() if isinstance(blocker_ref, str) else root / "__missing__"
        try:
            blocker_path.relative_to(root.resolve())
        except ValueError:
            blocker_path = root / "__missing__"
        blocker_data = load_json(blocker_path, errors) if blocker_path.is_file() else None
        fixture_preview_blocker_valid = (
            isinstance(blocker_data, dict)
            and isinstance(blocker_data.get("blockers"), list)
            and any(
                isinstance(item, dict) and item.get("status") == "BLOCKED-HIGHER-MATURITY"
                for item in blocker_data["blockers"]
            )
        )
        if not fixture_preview_blocker_valid:
            errors.append("fixture preview requires a packaged BLOCKED-HIGHER-MATURITY publication blocker artifact")
    ...
    for page_id in page_ids:
        ...
        independent_attestation_preview = (
        ...
        explicit_fixture_preview = (
            fixture_preview
            and fixture_preview_blocker_valid
            and manifest.get("validation_verdict") == "BLOCKED-HIGHER-MATURITY"
            and verdict == "FAIL"
            and isinstance(promotion.get("higher_maturity_blocker"), list)
            and len(promotion["higher_maturity_blocker"]) > 0
        )
        if ((verdict != "PASS" and not independent_attestation_preview and not explicit_fixture_preview)
                or (promotion.get("research_package_complete") is not True and not explicit_fixture_preview)):
    ...
    if manifest.get("validation_verdict") != "PASS" and not (
        fixture_preview
        and fixture_preview_blocker_valid
        and manifest.get("validation_verdict") == "BLOCKED-HIGHER-MATURITY"
    ):
