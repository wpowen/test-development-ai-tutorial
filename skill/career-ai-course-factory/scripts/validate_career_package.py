...
from run_claim_deep_research import extract_response_artifacts
...
REQUIRED_TUTORIAL_MARKDOWN = {
    "tutorial/README.md": ["## 如何使用", "## 教程结构", "## 当前完成度"],
    "tutorial/course-tree.md": ["## 文档导航", "## 模块", "## 页面状态"],
...
REQUIRED_TOPIC_RESEARCH_FILES = {
    ...
    "contradiction-matrix.md": ["## Claims"],
...
PROMOTION_RESEARCH_PACKAGE_COMMON_FILES = {
    ...
    "projection-ledger.json", "claim-inventory.json",
}
OPENAI_PROMOTION_RESEARCH_PACKAGE_FILES = PROMOTION_RESEARCH_PACKAGE_COMMON_FILES | {
    "deep-research-receipts.json", "contradiction-matrix.md", "research-saturation.json",
}
CODEX_PROMOTION_RESEARCH_PACKAGE_FILES = PROMOTION_RESEARCH_PACKAGE_COMMON_FILES | {
    "codex-research-receipts.json", "codex-contradiction-matrix.md", "codex-research-saturation.json",
}
REQUIRED_RESEARCH = [
...
DOCUMENT_TYPES = {"beginner-tutorial", "professional-how-to", "reference", "explanation", "decision-report"}
READER_JOBS = {"learn", "do", "look-up", "understand", "report-decide"}
DELIVERY_MODES = {"concept", "guided-lab", "diagnostic", "reference", "project"}
...
def validate_claim_deep_research(topic_dir: Path, page_id: str, errors: list[str]) -> None:
    inventory_path = topic_dir / "claim-inventory.json"
    inventory = load_json(inventory_path, errors) if inventory_path.is_file() else None
    if inventory is None:
        errors.append(f"tutorial promised page {page_id} missing or invalid claim-inventory.json")
        return
    if not isinstance(inventory, dict):
        errors.append(f"tutorial promised page {page_id} claim-inventory.json must be an object")
        return

    require_fields(inventory, ["schema_version", "topic_id", "extraction", "claims"], f"tutorial promised page {page_id} claim inventory", errors)
    if inventory.get("schema_version") != "claim-inventory.v1":
        errors.append(f"tutorial promised page {page_id} claim inventory has unsupported schema_version")
    if str(inventory.get("topic_id", "")) != page_id:
        errors.append(f"tutorial promised page {page_id} claim inventory topic_id mismatch")
    execution_contract = str(inventory.get("execution_contract", "openai-deep-research.v1"))
    if execution_contract not in {"openai-deep-research.v1", "codex-research.v1"}:
        errors.append(f"tutorial promised page {page_id} claim inventory has invalid execution_contract")

    extraction = inventory.get("extraction")
    author_id = ""
    auditor_id = ""
    if not isinstance(extraction, dict):
        errors.append(f"tutorial promised page {page_id} claim inventory missing extraction record")
    else:
        require_fields(extraction, ["author_id", "independent_auditor_id", "source_files", "source_hashes"], f"tutorial promised page {page_id} claim extraction", errors)
        author_id = str(extraction.get("author_id", ""))
        auditor_id = str(extraction.get("independent_auditor_id", ""))
        if author_id and author_id == auditor_id:
            errors.append(f"tutorial promised page {page_id} claim extraction auditor must be independent from author")
        if extraction.get("unmapped_propositions") != 0:
            errors.append(f"tutorial promised page {page_id} claim inventory unmapped_propositions must be 0")
        source_files = extraction.get("source_files", [])
        source_hashes = extraction.get("source_hashes", {})
        if not isinstance(source_files, list) or not source_files:
            errors.append(f"tutorial promised page {page_id} claim extraction needs source_files")
        elif not isinstance(source_hashes, dict):
            errors.append(f"tutorial promised page {page_id} claim extraction source_hashes must be an object")
        else:
            for relative in source_files:
                relative_text = str(relative)
                source_path = topic_dir / relative_text
                if not source_path.is_file() or source_path.stat().st_size == 0:
                    errors.append(f"tutorial promised page {page_id} claim extraction source is missing: {relative_text}")
                    continue
                expected_hash = source_hashes.get(relative_text)
                if expected_hash != file_sha256(source_path):
                    errors.append(f"tutorial promised page {page_id} claim extraction source hash mismatch: {relative_text}")
    ...
    errors.extend(f"tutorial promised page {page_id} locator contract: {item}" for item in locator_errors)

    claims = inventory.get("claims")
    claim_ids: set[str] = set()
    claim_dimensions: dict[str, set[str]] = {}
    if not isinstance(claims, list) or not claims:
        errors.append(f"tutorial promised page {page_id} claim inventory needs at least one claim")
    else:
        for index, claim in enumerate(claims):
            if not isinstance(claim, dict):
                errors.append(f"tutorial promised page {page_id} claim inventory row {index} must be an object")
                continue
            require_fields(claim, ["claim_id", "statement", "claim_type", "risk", "scope", "source_locations", "required_dimensions", "proposed_disposition"], f"tutorial promised page {page_id} claim {index}", errors)
            claim_id = str(claim.get("claim_id", ""))
            if claim_id in claim_ids:
                errors.append(f"tutorial promised page {page_id} duplicate claim_id: {claim_id}")
            if claim_id:
                claim_ids.add(claim_id)
            dimensions = claim.get("required_dimensions", [])
            claim_dimensions[claim_id] = {str(item) for item in dimensions} if isinstance(dimensions, list) else set()
            if claim.get("risk") not in {"low", "medium", "high", "critical"}:
                errors.append(f"tutorial promised page {page_id} claim {claim_id} has invalid risk")
            if claim.get("proposed_disposition") not in {"SUPPORTED", "SCOPED", "UNKNOWN-EXPLICIT", "REJECTED"}:
                errors.append(f"tutorial promised page {page_id} claim {claim_id} has invalid proposed_disposition")

    if execution_contract == "codex-research.v1":
        for finding in validate_codex_topic(topic_dir, require_saturation=True):
            errors.append(f"tutorial promised page {page_id} Codex research: {finding}")
        return

    receipts_path = topic_dir / "deep-research-receipts.json"
    receipts = load_json(receipts_path, errors) if receipts_path.is_file() else None
    if receipts is None:
        errors.append(f"tutorial promised page {page_id} missing or invalid deep-research-receipts.json")
        return
    if not isinstance(receipts, dict):
        errors.append(f"tutorial promised page {page_id} deep-research-receipts.json must be an object")
        return
    require_fields(receipts, ["schema_version", "topic_id", "runs"], f"tutorial promised page {page_id} deep research receipts", errors)
    if receipts.get("schema_version") != "deep-research-receipts.v1":
        errors.append(f"tutorial promised page {page_id} deep research receipts have unsupported schema_version")
    if str(receipts.get("topic_id", "")) != page_id:
        errors.append(f"tutorial promised page {page_id} deep research receipts topic_id mismatch")

    runs = receipts.get("runs")
    run_ids: set[str] = set()
    run_records: dict[str, dict[str, Any]] = {}
    initial_claims: set[str] = set()
    expansion_claims: set[str] = set()
    if not isinstance(runs, list) or not runs:
        errors.append(f"tutorial promised page {page_id} needs Deep Research runs")
        runs = []
    required_run_fields = [
        "run_id", "claim_ids", "round", "phase", "provider", "surface", "model_or_feature",
        "response_or_export_id", "started_at", "completed_at", "request_path", "raw_response_path",
        "report_path", "citations_path", "tool_calls_path", "input_sha256", "output_sha256",
        "data_sources", "tool_call_count", "citation_count", "opened_source_count", "status", "limitations",
    ]
    for index, run in enumerate(runs):
        if not isinstance(run, dict):
            errors.append(f"tutorial promised page {page_id} Deep Research run {index} must be an object")
            continue
        require_fields(run, required_run_fields, f"tutorial promised page {page_id} Deep Research run {index}", errors)
        run_id = str(run.get("run_id", ""))
        if run_id in run_ids:
            errors.append(f"tutorial promised page {page_id} duplicate Deep Research run_id: {run_id}")
        if run_id:
            run_ids.add(run_id)
            run_records[run_id] = run
        linked_claims = run.get("claim_ids", [])
        linked_claim_set = {str(item) for item in linked_claims} if isinstance(linked_claims, list) else set()
        if not linked_claim_set or not linked_claim_set.issubset(claim_ids):
            errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} references unknown claims")
        phase = str(run.get("phase", ""))
        if phase == "initial-deep-research":
            if len(linked_claim_set) != 1:
                errors.append(f"tutorial promised page {page_id} initial Deep Research run must cover exactly one claim: {run_id}")
            initial_claims.update(linked_claim_set)
        elif phase in {"counterevidence", "gap-fill", "verification"}:
            expansion_claims.update(linked_claim_set)
        ...
        if not data_sources.intersection({"web_search", "file_search", "remote_mcp"}):
            errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} lacks a research data source")
        for count_field in ("tool_call_count", "citation_count", "opened_source_count"):
            value = run.get(count_field)
            if not isinstance(value, int) or value < 1:
                errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} {count_field} must be >= 1")
        ...
        for path_field in path_fields:
            ...
            if candidate != topic_root and topic_root not in candidate.parents:
                errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} has unsafe {path_field}")
                continue
            resolved_paths[path_field] = candidate
            if not candidate.is_file() or candidate.stat().st_size == 0:
                errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} missing {path_field}: {relative_text}")
        request_path = resolved_paths.get("request_path")
        response_path = resolved_paths.get("raw_response_path")
        if request_path and request_path.is_file() and run.get("input_sha256") != file_sha256(request_path):
            errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} input_sha256 mismatch")
        if response_path and response_path.is_file() and run.get("output_sha256") != file_sha256(response_path):
            errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} output_sha256 mismatch")
        if run.get("surface") == "openai-responses-api" and response_path and response_path.is_file():
            raw_response = load_json(response_path, errors)
            if not isinstance(raw_response, dict):
                errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} raw response must be an object")
            else:
                if raw_response.get("id") != run.get("response_or_export_id"):
                    errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} raw response id mismatch")
                if raw_response.get("status") != "completed":
                    errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} raw response is not completed")
                raw_model = str(raw_response.get("model", ""))
                if not raw_model or raw_model != str(run.get("model_or_feature", "")):
                    errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} raw response model mismatch")
                raw_artifacts = extract_response_artifacts(raw_response)
                raw_counts = {
                    "tool_call_count": len(raw_artifacts["tool_calls"]),
                ...
                for count_field, raw_count in raw_counts.items():
                    if run.get(count_field) != raw_count:
                        ...
                        errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} raw response {label} mismatch")
                report_path = resolved_paths.get("report_path")
                if report_path and report_path.is_file():
                    report_text = report_path.read_text(encoding="utf-8").strip()
                    if report_text != str(raw_artifacts["report"]).strip():
                        errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} report does not match raw response output")
                citations_path = resolved_paths.get("citations_path")
                if citations_path and citations_path.is_file():
                    citations_doc = load_json(citations_path, errors)
                    saved_citations = citations_doc.get("citations") if isinstance(citations_doc, dict) else None
                    if not isinstance(saved_citations, list) or saved_citations != raw_artifacts["citations"]:
                        errors.append(f"tutorial promised page {page_id} Deep Research run {run_id} citations artifact does not match raw response")
                tool_calls_path = resolved_paths.get("tool_calls_path")
                if tool_calls_path and tool_calls_path.is_file():
                    calls_doc = load_json(tool_calls_path, errors)
    ...
    for claim_id in sorted(claim_ids - expansion_claims):
        errors.append(f"tutorial promised page {page_id} claim {claim_id} lacks counterevidence or gap-fill Deep Research")

    saturation_path = topic_dir / "research-saturation.json"
    saturation = load_json(saturation_path, errors) if saturation_path.is_file() else None
    if saturation is None:
        errors.append(f"tutorial promised page {page_id} missing or invalid research-saturation.json")
        return
    if not isinstance(saturation, dict):
        errors.append(f"tutorial promised page {page_id} research-saturation.json must be an object")
        return
    require_fields(saturation, ["schema_version", "topic_id", "independent_auditor_id", "claims", "overall_verdict"], f"tutorial promised page {page_id} research saturation", errors)
    if saturation.get("schema_version") != "research-saturation.v1" or saturation.get("overall_verdict") != "PASS":
        errors.append(f"tutorial promised page {page_id} research saturation verdict is not PASS")
    if str(saturation.get("topic_id", "")) != page_id:
        errors.append(f"tutorial promised page {page_id} research saturation topic_id mismatch")
    if str(saturation.get("independent_auditor_id", "")) in {"", author_id}:
        errors.append(f"tutorial promised page {page_id} research saturation auditor must be independent")
    saturation_rows = saturation.get("claims", [])
    saturation_ids: set[str] = set()
    if not isinstance(saturation_rows, list):
        errors.append(f"tutorial promised page {page_id} research saturation claims must be an array")
        saturation_rows = []
    for index, row in enumerate(saturation_rows):
        if not isinstance(row, dict):
            errors.append(f"tutorial promised page {page_id} research saturation row {index} must be an object")
            continue
        require_fields(row, ["claim_id", "run_ids", "coverage_dimensions", "contradiction_status", "two_consecutive_rounds_without_material_change", "conclusive_primary_authority_exception", "round_assessments", "final_disposition", "verdict", "rationale"], f"tutorial promised page {page_id} research saturation row {index}", errors)
        claim_id = str(row.get("claim_id", ""))
        saturation_ids.add(claim_id)
        linked_runs = {str(item) for item in row.get("run_ids", [])} if isinstance(row.get("run_ids"), list) else set()
        if len(linked_runs) < 2 or not linked_runs.issubset(run_ids):
            errors.append(f"tutorial promised page {page_id} claim {claim_id} saturation must reference at least two valid Deep Research runs")
        coverage = row.get("coverage_dimensions", [])
        coverage_map: dict[str, str] = {}
        if isinstance(coverage, list):
            for item in coverage:
                if isinstance(item, dict) and item.get("dimension") and item.get("evidence_or_reason"):
                    coverage_map[str(item["dimension"])] = str(item.get("status", ""))
        missing_dimensions = claim_dimensions.get(claim_id, set()) - set(coverage_map)
        if missing_dimensions:
            errors.append(f"tutorial promised page {page_id} claim {claim_id} saturation missing dimensions: {', '.join(sorted(missing_dimensions))}")
        if any(status not in {"covered", "not-applicable"} for status in coverage_map.values()):
            errors.append(f"tutorial promised page {page_id} claim {claim_id} saturation has invalid dimension status")
        ...
        if not isinstance(assessments, list) or len(assessments) != len(linked_runs):
            errors.append(f"tutorial promised page {page_id} claim {claim_id} needs one round assessment per linked run")
        else:
            for assessment in assessments:
                if not isinstance(assessment, dict):
                    errors.append(f"tutorial promised page {page_id} claim {claim_id} has invalid round assessment")
                    continue
                assessment_run_id = str(assessment.get("run_id", ""))
                if assessment_run_id not in linked_runs or assessment_run_id in assessment_map:
                    errors.append(f"tutorial promised page {page_id} claim {claim_id} round assessment references an invalid or duplicate run")
                    continue
                if not isinstance(assessment.get("material_change"), bool) or not str(assessment.get("assessment", "")).strip():
                    errors.append(f"tutorial promised page {page_id} claim {claim_id} round assessment lacks material-change evidence")
                    continue
                assessment_map[assessment_run_id] = assessment
        ordered_linked_runs = sorted(
            (run_records[run_id] for run_id in linked_runs if run_id in run_records),
            key=lambda item: int(item.get("round", 0)) if isinstance(item.get("round"), int) else 0,
        )
        ordered_linked_ids = [str(item.get("run_id", "")) for item in ordered_linked_runs]
        ordered_round_numbers = [
            int(item["round"]) for item in ordered_linked_runs if isinstance(item.get("round"), int)
        ]
        if len(ordered_round_numbers) != len(ordered_linked_runs) or len(set(ordered_round_numbers)) != len(ordered_round_numbers):
            errors.append(f"tutorial promised page {page_id} claim {claim_id} linked run rounds must be valid and unique")
        last_two_stable = len(ordered_linked_ids) >= 2 and all(
            assessment_map.get(run_id, {}).get("material_change") is False for run_id in ordered_linked_ids[-2:]
        )
        if stable != last_two_stable:
            errors.append(f"tutorial promised page {page_id} claim {claim_id} stable-round assertion does not match the last two round assessments")
        if stable and len(ordered_round_numbers) >= 2 and ordered_round_numbers[-1] != ordered_round_numbers[-2] + 1:
            errors.append(f"tutorial promised page {page_id} claim {claim_id} stable rounds must be consecutive")
        if primary_exception and not str(row.get("primary_authority_exception_rationale", "")).strip():
            errors.append(f"tutorial promised page {page_id} claim {claim_id} primary-authority exception lacks rationale")
        if not stable and not primary_exception:
            errors.append(f"tutorial promised page {page_id} claim {claim_id} saturation lacks two stable rounds or a primary-authority exception")
        if row.get("verdict") != "SATURATED":
            errors.append(f"tutorial promised page {page_id} claim {claim_id} research is not SATURATED")
        if row.get("final_disposition") not in {"SUPPORTED", "SCOPED", "UNKNOWN-EXPLICIT", "REJECTED"}:
            errors.append(f"tutorial promised page {page_id} claim {claim_id} has invalid final_disposition")
    if saturation_ids != claim_ids:
        errors.append(f"tutorial promised page {page_id} research saturation claim set does not match claim inventory")
    contradiction_path = topic_dir / "contradiction-matrix.md"
    if contradiction_path.is_file():
        contradiction_text = contradiction_path.read_text(encoding="utf-8")
        for claim_id in sorted(claim_ids):
            if claim_id not in contradiction_text:
                errors.append(f"tutorial promised page {page_id} contradiction matrix is missing claim {claim_id}")

    projection_path = topic_dir / "projection-ledger.json"
    projection = load_json(projection_path, errors) if projection_path.is_file() else None
    if isinstance(projection, dict):
        ...
        if not projected_ids.issubset(claim_ids):
            errors.append(f"tutorial promised page {page_id} projection ledger contains claims absent from claim inventory")
...
def validate_topic_research(root: Path, page_id: str, errors: list[str]) -> None:
    ...
    validate_claim_deep_research(topic_dir, page_id, errors)
...
def validate_catalog_publication_contract(root: Path, errors: list[str]) -> None:
    ...
    for page_id in public_ids:
        ...
        claim_inventory_path = root / "research/topics" / page_id / "claim-inventory.json"
        claim_inventory = load_json(claim_inventory_path, errors) if claim_inventory_path.is_file() else {}
        execution_contract = str(claim_inventory.get("execution_contract", "openai-deep-research.v1")) if isinstance(claim_inventory, dict) else "openai-deep-research.v1"
        expected_inventory = (
            CODEX_PROMOTION_RESEARCH_PACKAGE_FILES
            if execution_contract == "codex-research.v1"
            else OPENAI_PROMOTION_RESEARCH_PACKAGE_FILES
        )
        if inventory != expected_inventory:
            errors.append(f"tutorial promised page {page_id} promotion receipt must inventory the exact fourteen-file research package")
...
def validate_cluster_route_manifests(root: Path, errors: list[str]) -> None:
    """Consume any clustered route manifest instead of treating it as a note.

    The route manifest is a pre-research gate.  A valid ``BLOCKED`` manifest is
    useful evidence, but it must keep the package blocked; an invalid or
    malformed manifest must never be silently ignored by the package validator.
    Fixture packages without clustered research artifacts remain compatible.
    """
    research_root = root / "research"
    candidates = sorted(research_root.glob("research-cluster-redesign-*/research-route-dry-run*.json"))
    clustered_inputs = list(research_root.glob("claim-source-manifest-*.json"))
    if clustered_inputs and not candidates:
        errors.append("clustered claim-source manifest exists without a route dry-run manifest")
        return
    required = {"schema_version", "dry_run_id", "generated_at", "input_inventory_digests", "map_digest", "status", "counts", "routes", "reuse", "invalidation", "unclassified_count"}
    for path in candidates:
        label = f"cluster route manifest {path.relative_to(root)}"
        data = load_json(path, errors)
        if not isinstance(data, dict):
            continue
        ...
        if missing:
            ...
            continue
        if data.get("schema_version") != "research-route-dry-run-manifest.v1":
            errors.append(f"{label} has unsupported schema_version")
        if data.get("status") != "READY":
            errors.append(f"{label} status is {data.get('status')!r}; research queue is not release-ready")
        counts = data.get("counts")
        routes = data.get("routes")
        if not isinstance(counts, dict) or not isinstance(routes, dict):
            errors.append(f"{label} counts/routes must be objects")
            continue
        if counts.get("missing_page_count") != 0 or counts.get("unexpected_page_count") != 0:
            errors.append(f"{label} page coverage is incomplete")
        if routes.get("BLOCKED-UNCLASSIFIED", 0) != 0 or data.get("unclassified_count") != 0:
            errors.append(f"{label} contains unclassified claims")


def validate_research(root: Path, errors: list[str]) -> None:
    research = root / "research"
    validate_cluster_route_manifests(root, errors)
...
def validate_tutorial(root: Path, errors: list[str]) -> None:
    ...
    page_fields = [
        "page_id", "slug", "module_id", "title", "document_type", "reader_job", "audience",
        "scope", "claims", "document_contract", "delivery_mode", "level", "order", "display_number",
    ...
    for index, page in enumerate(pages):
        ...
        reader_job = page.get("reader_job")
        if document_type not in DOCUMENT_TYPES:
            errors.append(f"tutorial page {index} has invalid document_type")
        if reader_job not in READER_JOBS:
            errors.append(f"tutorial page {index} has invalid reader_job")
        if page.get("delivery_mode") not in DELIVERY_MODES:
            errors.append(f"tutorial page {index} has invalid delivery_mode")
        if not isinstance(page.get("audience"), list) or not page.get("audience"):
            errors.append(f"tutorial page {index} audience must be a non-empty list")
        scope = page.get("scope")
        if not isinstance(scope, dict) or not all(isinstance(scope.get(key), list) and scope.get(key) for key in ("inScope", "outOfScope", "assumptions")):
            errors.append(f"tutorial page {index} scope must define non-empty inScope, outOfScope and assumptions")
        if not isinstance(page.get("claims"), list) or not page.get("claims"):
            errors.append(f"tutorial page {index} claims must be a non-empty list")
        document_contract = page.get("document_contract")
        if not isinstance(document_contract, dict):
            errors.append(f"tutorial page {index} document contract must be an object")
        else:
            if document_contract.get("documentType") != document_type or document_contract.get("readerJob") != reader_job:
                errors.append(f"tutorial page {index} document contract type/job mismatch")
            required_contract_field = {
                "beginner-tutorial": "learningContract",
                "professional-how-to": "procedure",
                "reference": "referenceContract",
                "explanation": "explanationContract",
                "decision-report": "reportContract",
            }.get(str(document_type))
            if required_contract_field and not isinstance(document_contract.get(required_contract_field), dict):
                errors.append(f"tutorial page {index} document contract lacks {required_contract_field}")
    ...
    if not isinstance(release_scope, dict):
        ...
        if mode not in {"validated-subset", "complete-catalog"}:
            errors.append("tutorial release_scope mode must be validated-subset or complete-catalog")
        ...
        if mode == "complete-catalog":
            ...
            if len(pages) < 8:
                errors.append("validated-subset public tutorial needs enough delivered pages to close one task coverage set")
            if release_scope.get("catalog_complete") is not False:
                errors.append("validated-subset release_scope must set catalog_complete=false")

    html = html_path.read_text(encoding="utf-8")
    if len(html.strip()) < 8000:
        errors.append("tutorial/index.html is too thin to provide the learning experience")
    for marker in ["id=\"course-nav\"", "id=\"tutorial-content\"", "id=\"page-toc\"", "id=\"tutorial-search\"", "id=\"progress-bar\""]:
        if marker not in html:
            errors.append(f"tutorial/index.html missing viewer marker: {marker}")
    if not any(marker in html for marker in ("COURSE_DATA", "COURSE_INDEX", "course-index.json")):
        errors.append("tutorial/index.html missing viewer data marker: COURSE_DATA, COURSE_INDEX or course-index.json")
    lowered = html.lower()
