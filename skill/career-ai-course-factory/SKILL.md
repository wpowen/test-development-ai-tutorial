...
3. **R1–R4 canonical external research**：按规范化 request fingerprint 去重，只为合资格的 canonical anchor 运行显式选择的 `openai-deep-research.v1` 或 `codex-research.v1`；两个 execution contract 的 receipt、身份、opened-source 和饱和语义完全分离，不能混用来借成熟度。按适用风险执行 initial、技术文档重建、counterevidence 和 gap-fill。anchor 可产生 N2 canonical knowledge packet，但 packet 是共享证据资产，不是多个 claim 的 provider receipt。
...
对于 OpenAI Responses API，使用 `scripts/run_claim_deep_research.py` 仅执行已批准的 canonical anchor 或命名的 delta/gap run。模型通过 `--model` 或 `OPENAI_DEEP_RESEARCH_MODEL` 在运行时解析；不要硬编码已废弃模型。公开 web 与私有 file search/remote MCP 默认分离，混合需显式风险审阅。runner 必须把 `discovered_urls`、`cited_urls` 与明确 `open_page/open/browse` 事件分开；搜索候选或 citation 永远不能推导 opened。每个 completed API run 必须保存 request、raw response、report、citations、tool calls、`source-opening-ledger.json`、hash、provider response ID 和 receipt；缺任一项就保留诊断并返回 `BLOCKED-DEEP-RESEARCH`，不得写入 completed receipt。TD-T09 已有历史 receipt 原样保留，不重写、不复制、不升级。

当 claim inventory 显式选择 `execution_contract=codex-research.v1` 时，严格执行 `references/codex-research-execution-contract.md`：至少两名 runtime-observed 独立证据 agent、独立 context/session/group、fresh counterevidence、第三身份 adjudicator、citation→source-open event 映射、签名 runtime trace 与独立 saturation。成功状态只能是 `PASS-CODEX-RESEARCH`，同时 `deep_research_status=BLOCKED-DEEP-RESEARCH`；integrator、普通 Luna 对话、共享 context、伪 agent label 或 citation-only 轨迹都不能计数。
