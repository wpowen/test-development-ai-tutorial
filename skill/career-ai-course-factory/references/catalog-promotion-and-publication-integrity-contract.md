...
- A `pilot-path` public set must be a subset of this catalog.
...
Every candidate public page must first pass its ten-file topic package (the original nine research/manuscript files plus `projection-ledger.json`):

`research-brief.md`, `source-pack.csv`, `research-runs.json`, `evidence-synthesis.md`, `engineering-blueprint.md`, `manuscript.md`, `comparison.md`, `lab-manifest.json`, and `validation.md`.

Its `promotion-receipt.json` then records the exact ten-file inventory, `PASS` verdict, editorial score of at least 90, boundary-preservation score of 100, the current executability-audit path and hash, exact learner-material hashes, validation time, and reviewer. The receipt belongs to one exact page ID.
...
3. ten-file topic package including research-to-page projection;
