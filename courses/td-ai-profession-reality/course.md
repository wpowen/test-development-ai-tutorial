...
学员最终交付：职业责任地图；需求/技术文档双栏阅读表；冲突 BLOCK 记录；风险—方法—独立 Oracle 矩阵；工件 owner/version/source/acceptance/consumer 表；AI 能力迁移矩阵；版本化职业重建 Prompt 包；以及带 baseline/fault/repair 报告的证据缺口行动计划。每个 artifact 都要有消费者和下一项决定，不能只提交一篇心得。
