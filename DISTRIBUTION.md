...
- 本轮 103 页修改尚未部署；上述地址只代表历史 fixture preview，不能作为当前版本在线证据。
...
截至 2026-08-18，当前内容源投影为 103 页、13 个公共模块，另有 4 个内部未完成命题。公共集合覆盖职业与需求生命周期、传统测试专项、AI 基础、AI 辅助测试、AI 服务性能可靠性、LLM/RAG、Agent/Workflow、质量系统、Benchmark、质量平台、Capstone、职业迁移和高级质量缺口。

这 103 页通过了当前内容结构、语义、静态投影与 fixture 测试，但逐命题 Deep Research、独立 promotion、executability 和 publication closure 尚未针对当前哈希闭合。历史文档中的 17、33、44、52、65、85 或 102 页均为旧快照；当前事实以 `site/content/course.ts`、`research/catalog-manifest.json`、`tutorial/tutorial-site.json` 和 `tutorial/fidelity-manifest.json` 的一致结果为准。

每个任务文档都要求从输入依据、风险与方法、独立 Oracle、Prompt/Eval/Mutation、运行收据到发布判断形成闭环。内部知识树继续维护其他命题；未通过逐题研究与教材门禁的内容不得进入新的正式发布。
...
- 当前 103 页修改尚未部署到 GitHub Pages 或 ChatGPT Site；
...
- 当前静态站 ZIP：尚未在新发布闭包通过前生成；
- GitHub 仓库候选目录：`dist/github-candidate/`；
- GitHub 仓库候选 ZIP：尚未在新发布闭包通过前生成；
- 机器清单：`dist/PUBLICATION-CANDIDATES.json`。

新的 GitHub 候选包必须包含当前 103 页、16 个材料包、GitHub Actions、Skill、课程实验以及 `SOLUTION-MANIFEST.json`、`CATALOG-MANIFEST.json`、`PAGE-PROMOTION-MANIFEST.json`、`EXECUTABILITY-MANIFEST.json`、`ARTIFACT-CLOSURE.json`。在逐命题研究、独立审阅和当前哈希闭包完成前，不得生成可发布候选；更高成熟度仍需真实集成、具名从业者审批和公开发布回读。
